...
export async function POST(request) {
  try {
    ...
    if (contentType.includes("multipart/form-data")) {
      try {
        ...
        for (const [key, value] of formData.entries()) {
          console.log(`📁 فیلد: ${key} = ${typeof value === "object" ? "File" : value}`);
        ...
        if (videoFile && videoFile.size > 0) {
          console.log(`🎬 فیلم دریافت شد: ${videoFile.name}, سایز: ${videoFile.size} bytes, نوع: ${videoFile.type}`);

          // بررسی حجم (حداکثر ۲۰ مگابایت)
          if (videoFile.size > 20 * 1024 * 1024) {
            return NextResponse.json(
              { success: false, message: "حجم فیلم نباید بیشتر از ۲۰ مگابایت باشد" },
              { status: 400 }
            );
          }
          ...
          if (!allowedTypes.includes(videoFile.type)) {
            return NextResponse.json(
              { success: false, message: "فرمت فیلم پشتیبانی نمی‌شود. فرمت‌های مجاز: MP4, WebM, OGG, MOV" },
              { status: 400 }
            );
          }
          ...
          const uploadDir = path.join(process.cwd(), "public/uploads/exercises/videos");
        ...
        return NextResponse.json(
          { success: false, message: "خطا در پردازش فایل: " + formError.message },
          { status: 400 }
        );
      }
    } else {
    ...
    if (typeof body.muscles === "string") {
      body.muscles = body.muscles.split(",").map((m) => m.trim()).filter(Boolean);
    }
